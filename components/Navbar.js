        ...
        <>
            <nav style={{ background: "var(--nav-bg)", borderColor: "var(--nav-border)" }}
                ...
                <div className="flex md:hidden items-center gap-3">
                    ...
                    <button
                        onClick={handleToggle}
                        aria-label="Toggle theme"
                        className={`relative w-12 h-6 rounded-full border-2 border-white transition-all duration-500 cursor-pointer focus:outline-none ${isDark ? "bg-[#0f0f23]" : "bg-sky-300"}`}
                    >
                        <span className={`absolute top-0.5 w-4 h-4 rounded-full shadow-md flex items-center justify-center text-xs transition-all duration-500 ease-in-out ${isDark ? "translate-x-6 bg-gray-200" : "translate-x-0.5 bg-yellow-300"}`}>
                            {isDark ? (
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-2.5 h-2.5 text-indigo-900">
                                    <path fillRule="evenodd" d="M9.528 1.718a.75.75 0 01.162.819A8.97 8.97 0 009 6a9 9 0 009 9 8.97 8.97 0 003.463-.69.75.75 0 01.981.98 10.503 10.503 0 01-9.694 6.46c-5.799 0-10.5-4.701-10.5-10.5 0-4.368 2.667-8.112 6.46-9.694a.75.75 0 01.818.162z" clipRule="evenodd" />
                                </svg>
                            ) : (
            ...
            {mobileSearchOpen && (
                <div
                    ...
                    className="md:hidden flex items-center gap-2 px-4 py-3 border-b border-[#00ccff]"
                >
                    <input
                        type="text"
                        placeholder="Search moods or places..."
                        ...
                        onKeyDown={handleKeyPress}
                        autoFocus
                        className="flex-1 outline-none px-3 py-2 rounded-lg bg-[#ffcc5c] placeholder-white/70 text-white text-sm"
                    />
                    <button
                        onClick={handleSearch}
                        className="px-4 py-2 bg-white text-[#00aacc] rounded-lg text-sm font-semibold cursor-pointer"
                    >
                        Search
                    </button>
                    <button
                        onClick={() => setMobileSearchOpen(false)}
                        className="text-white text-lg cursor-pointer px-1"
                    >
                        ✕
                    </button>
                </div>
